...
import type { Sentence, Song } from '@/types'
...
const currentSong = computed<Song>(() => store.currentSong)
...
function doKaraoke() {
  const currentTime = store.howler.seek() * 1000
  if (currentTime < sections[0].start - 4000) drawIntro()
  else draw(store.howler.seek() * 1000)
...
function draw(seek: number) {
  ...
  sentences.forEach(({ text, start, end, words, index, alpha }: any) => {
    ...
    const contentHeight = fontSize * lines + lineHeight * fontSize * (lines - 1)
...
<template>
  <div>
    <h4
      ...
      class="absolute left-1/2 -translate-x-1/2 transform text-center text-2xl font-bold text-white"
    >
      {{ countdown }}
    </h4>
    <canvas
